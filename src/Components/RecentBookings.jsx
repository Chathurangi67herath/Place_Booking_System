...
export default function RecentBookings() {
  ...
  // Filter bookings by hall name based on search term
  const filteredData = datas.filter((data) =>
    data.hall.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <>
      <NavBar />
      <Box ml={{ base: 0 }} bg={useColorModeValue("gray.100", "gray.900")} p="4" minH={"85vh"}>
        <Box px="7" pt="3" pb="12" borderRadius="xl" bg={useColorModeValue("white", "gray.500")}>
          <Input
          ...
          {filteredData.length === 0 ? (
            <Center>
              <VStack>
                <Spinner thickness="3px" speed="0.65s" size="lg" emptyColor="gray.200" color="blue.500" />
                <Text>Data is loading. Please wait...</Text>
              </VStack>
            </Center>
          ) : (
